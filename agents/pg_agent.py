...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        # DONE TODO: return the estimated qvals based on the given rewards, using
        ...
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
            ...
            n=rewards_list.shape[0]
            q_values=np.concatenate([self._discounted_cumsum(rewards_list[i]) for i in range(n)])
    ...
    def estimate_advantage(self, obs, rews_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            if self.gae_lambda is not None:
                ...
                ## DONE TODO: compute advantage estimates using q_values, and values as baselines
    ...
    def _discounted_return(self, rewards):
        ...
        # DONE TODO: create list_of_discounted_returns
        T=len(rewards)
        gamma=np.array([(self.gamma)**i for i in range(T)]).squeeze()
        rewards=np.array(rewards).squeeze()
        assert rewards.shape==gamma.shape
        list_of_discounted_returns=[np.sum(rewards*gamma) for i in range(T)]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        cumsum=0    
        for i in range(T):
          cumsum=rewards[T-1-i]+self.gamma*cumsum
          list_of_discounted_cumsums[T-1-i]=cumsum
